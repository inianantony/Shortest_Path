    ...
    public class RawStationConvertor
    {
        public List<Station> Convert(List<RawStationData> rawRecords)
        {
            ...
            foreach (var rawStationData in rawRecords)
            {
                ...
                if (stations.Exists(a => a.IsSameAs(rawStationData.StationName)))
                {
                    station = stations.First(a => a.IsSameAs(rawStationData.StationName));
                }
                else
                {
            ...
            List<Station> stations)
        {
            return rawRecords.GroupBy(
                    a => a.Line,
                    b => stations.First(c => c.IsSameAs(b.StationName)))
                .ToDictionary(
                    a => a.Key,
                    b => b.ToList());
        }
